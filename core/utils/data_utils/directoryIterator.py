...
class loadDataset(Dataset):
    def __init__(self,cfg,
        ...
        try:
            ...
            with open(txtdir.replace('train','all').replace('test','all').replace('val','all')) as f:
                Files = [tuple(line) for line in csv.reader(f)]
            Files = np.array(Files)
        image_suffix=os.listdir(os.path.join(data_basedir, 'image', 'before'))[0].split('.')[-1]
        label_suffix = os.listdir(os.path.join(data_basedir, 'label', 'before'))[0].split('.')[-1]
        for i in range(len(Files)):
            ...
            self.clabFiles.append(os.path.join(data_basedir, 'label', 'change', fname))
        ...
        for i in range(len(Files)):
            if i > 500:
                break
            img = cv2.imread(os.path.join(data_basedir, 'image', Files[i][0]) + '.' + image_suffix)
    ...
    def __getitem__(self, index):
        ...
        chg_label = np.array(Image.open(self.clabFiles[index]).convert('L'))
        ...
        label_temp = np.zeros((chg_label.shape[0], chg_label.shape[1]))
        for num, color_idx in enumerate(self.COLOR_MAP):
            label_temp += np.ones((chg_label.shape[0], chg_label.shape[1])) * (chg_label == color_idx) * num
        chg_label = label_temp
        ...
        if self.transform is not None:
            ...
            temp,chg_label,temp = self.transform.apply_transform(np.ones_like(after_image),chg_label,transformer,random_num=random_num,
                                                                        training=self.training)
