...
class SCRUnet101 (nn.Module):
    def __init__(self,
        ...
        self._up_kwargs = {'mode': 'bilinear', 'align_corners': True}
        self.psp=PSPModule(8+filters[0])
        self.firstconv = resnet.conv1
        self.firstbn = resnet.bn1
        self.firstrelu = resnet.relu
        self.firstmaxpool = resnet.maxpool
        ...
        for i in range(len(predict)):
            if i==0:
                ...
                if self.training:
                    ...
                    return self.criterion(predict[0].squeeze(), (gts[0]>0).squeeze().float())
...
class MuiltiSCRUnet(nn.Module):
    def __init__(self,
        ...
        self.criterion=criterion
        filters = [64, 128, 256, 512]
        self.beforeUnet=SCRUnet(1,criterion)
        self.afterUnet=SCRUnet(1,criterion)
    ...
    def cal_loss(self,predict,gts,trainset):
        ...
        # for i in range(amask.shape[0]):
        #     amask[i]=amask[i]*trainset[i]
        # loss=self.bce(bpredict_seg.squeeze(),bmask.squeeze().float())+self.bce(bpredict_edge.squeeze(),bedge.squeeze().float())
        if self.training:
            bloss = self.bce(bpredict_seg.squeeze(), bmask.squeeze().float()).mean((1, 2)) + self.bce(
                bpredict_edge.squeeze(), bedge.squeeze().float()).mean((1, 2))
            tloss= self.bce(apredict_seg.squeeze(), amask.squeeze().float()).mean((1,2)) + self.bce(
                apredict_edge.squeeze(),aedge.squeeze().float()).mean((1,2))
            bmask = bmask.float()
            bmask[bmask == 0] = -1
            bmask[bmask > 0] = 1
            bmask = bmask * 3#default:5 set to 5 in 07/12
            ...
            # vloss+=self.mse(amid,fmid).mean((1,2,3))
            floss=bloss+tloss*trainset.float()+vloss*(1-trainset.float())
        else:
            bmask = bmask.float()
            bmask[bmask == 0] = -1
            bmask[bmask > 0] = 1
    ...
    def initialize_weights(self):
        pth=torch.load('/public/home/ghn/BuildingChange_Pytorch/models/SCRUNet_bval_0414.pth')
        net_state_dict=self.beforeUnet.state_dict()
        pretrained_dict = {k: v for k, v in pth.items() if k in net_state_dict}
        net_state_dict.update(pretrained_dict)
        self.beforeUnet.load_state_dict(pth)
        self.afterUnet.load_state_dict(pth)
        print('Initialize success!')
    def forward(self, x,gts,trainset):
        loss1,pred1=self.beforeUnet(x[0],gts[0],True)
        ...
        return loss1+loss,pred


if __name__=="__main__":
    img=np.random.rand(1,3,256,256)
    x=torch.tensor(img,dtype=torch.float32)
    a=SCRUnet101(1,None).forward(x,x)
